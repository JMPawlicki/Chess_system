...
It is normally started from the MATLAB GUI using the **Start Server** button. It can also be started manually:
...
The normal backend can observe DGT updates during robot movement. After `ROBOT_DONE`, it compares the physical DGT mirror board with the expected board after the pending robot move.

If the position does not match, the backend sends:
