...
# NOWE
ROBOT_SEGMENT_TIMEOUT = 0.6
ROBOT_DONE_DGT_SETTLE = 0.8
...
class GameSession:
    ...
    def handle_dgt_segments(self, segments):
        ...
        with self._lock:
            if self._state == STATE_WAITING_ENGINE_DONE:
                self._observe_robot_dgt_sequence(segments)
                return
    ...
    def _apply_robot_segments_to_mirror(self, current_segments):
        ...
        It is intentionally similar to _process_dgt_segments(), but it never calls
        _apply_human_move() and never changes python-chess state.
        """
        ...
        if n == 3:
            ...
            #   attacker removed,
            #   attacker placed.
            captured_idx = current_segments[0][3]
...
def dgt_thread(session, stop_event):
    ...
    while not stop_event.is_set():
        ...
        #NOWE
        segment_timeout = ROBOT_SEGMENT_TIMEOUT if session.is_waiting_engine_done() else SEGMENT_TIMEOUT
        # segment timeout zamiast SEGMENT_TIMEOUT 
