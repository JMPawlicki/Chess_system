...
% --------------------------
% OLD = previous board calibration used when regions were created.
% NEW = currently measured board calibration after restart/check.
%
% The regions are still stored in OLD coordinates. A global correction is
% applied below after the region database is defined, so you do not need to
% manually recalibrate every region point.

applyBoardRecalibration = true;

% Previous calibration
P_A1_center_old = [-165, -715, 25];
P_H1_center_old = [ 225, -708, 25];
P_A8_center_old = [-165, -330, 21];
P_H8_center_old = [ 211, -317, 19];

% Current calibration measured on the robot [mm]
P_A1_center_new = [-177, -709.0, 24.5];
P_H1_center_new = [ 208, -709.5, 23.0];
P_A8_center_new = [-175, -323, 19.0];
P_H8_center_new = [ 209, -325.0, 18.5];

% Square center from old board calibration
squareCenterOld = @(fileIdx, rankIdx) ...
    (1-fileIdx/7)*(1-rankIdx/7)*P_A1_center_old + ...
    (fileIdx/7)*(1-rankIdx/7)*P_H1_center_old + ...
    (1-fileIdx/7)*(rankIdx/7)*P_A8_center_old + ...
    (fileIdx/7)*(rankIdx/7)*P_H8_center_old;

% Square center from current board calibration
squareCenterNew = @(fileIdx, rankIdx) ...
    (1-fileIdx/7)*(1-rankIdx/7)*P_A1_center_new + ...
    (fileIdx/7)*(1-rankIdx/7)*P_H1_center_new + ...
    (1-fileIdx/7)*(rankIdx/7)*P_A8_center_new + ...
    (fileIdx/7)*(rankIdx/7)*P_H8_center_new;

% Main squareCenter used later in the code.
% If applyBoardRecalibration = true, all generated deltas use the NEW board.
squareCenter = squareCenterNew;

%% -------------------------
%  Robot TCP [m, rad]
% --------------------------
% Explicitly set TCP at the beginning of each URScript block. This prevents
% unexpected behaviour if the active TCP is changed/reset outside MATLAB.
TCP_GRIPPER = [0.000000, -0.290000, 0.260000, 0.000000, 0.000000, 0.000000];
TCP_GRIPPER_CMD = sprintf("  set_tcp(p[%.6f, %.6f, %.6f, %.6f, %.6f, %.6f])" + newline, ...
    TCP_GRIPPER(1), TCP_GRIPPER(2), TCP_GRIPPER(3), ...
    TCP_GRIPPER(4), TCP_GRIPPER(5), TCP_GRIPPER(6));
...
%% -------------------------
%  Apply board recalibration to all stored region points
% --------------------------
% Region P_* points were saved as absolute Cartesian points for each
% region's baseSquare. If the board changed slightly, update each region by
% the difference between old and new baseSquare center. Orientations are not
% modified.

if applyBoardRecalibration
    fieldsToShift = [ ...
        "P_APPROACH_HIGH", ...
        "P_APPROACH_MID", ...
        "P_APPROACH_LOW", ...
    ...
    fprintf("\nApplying board recalibration to regions...\n");

    for k = 1:numel(regions)
        base = char(upper(regions(k).baseSquare));

        baseFileIdx = double(base(1) - 'A');
        baseRankIdx = str2double(base(2)) - 1;

        C_old = squareCenterOld(baseFileIdx, baseRankIdx);
        C_new = squareCenterNew(baseFileIdx, baseRankIdx);
        delta_board_mm = C_new - C_old;

        fprintf("%s base %s delta [mm] = [%.2f, %.2f, %.2f]\n", ...
            regions(k).name, regions(k).baseSquare, ...
            delta_board_mm(1), delta_board_mm(2), delta_board_mm(3));

        for f = 1:numel(fieldsToShift)
            fieldName = char(fieldsToShift(f));

            if isfield(regions(k), fieldName) && ~isempty(regions(k).(fieldName))
                regions(k).(fieldName)(1:3) = regions(k).(fieldName)(1:3) + delta_board_mm;
            end
        end
    end
end
...
script = script + sprintf( ...
    ...
    Q_HOME_lin(1), Q_HOME_lin(2), Q_HOME_lin(3), Q_HOME_lin(4), Q_HOME_lin(5), Q_HOME_lin(6), ...
    a_lin, v_lin);
